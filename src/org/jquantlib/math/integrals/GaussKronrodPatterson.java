...
public class GaussKronrodPatterson extends Integrator {
 ...
 /* x1, abscissae common to the 10-, 21-, 43- and 87-point rule */
    ...
    public GaussKronrodPatterson(final double relativeAccuracy, final double absoluteAccuracy) {
    ...
    public final void setRelativeAccuracy(final double relativeAccuracy) {
        this.relativeAccuracy = relativeAccuracy;
    }
    //
    ...
    protected double integrate(final Ops.DoubleOp f, final double a, final double b) {
        ...
        for (int k = 0; k < 5; k++) {
            resasc += (w21a[k]
                    * (Math.abs(fv1[k] - mean) + Math.abs(fv2[k] - mean)) + w21b[k]
        ...
        for (int k = 0; k < 11; k++) {
            ...
            final double fval = (f.op(center + abscissa) + f.op(center
                    - abscissa));
            res43 += fval * w43b[k];
            savfun[k + 10] = fval;
        }
    ...
    private double rescaleError(double err, final double resultAbs, final double resultAsc) {
        ...
        if (resultAbs > Constants.QL_MIN_POSITIVE_REAL
                / (50 * Constants.QL_EPSILON)) {
