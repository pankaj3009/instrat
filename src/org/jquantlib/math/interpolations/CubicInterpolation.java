 ...
 * <p>
 * Cubic interpolation is fully defined when the ${f_i}$ function values at
 * points ${x_i}$ are supplemented with ${f_i}$ function derivative values.
 * <p>
 * Different type of first derivative approximations are implemented, both local
 * and non-local. Local schemes (Fourth-order, Parabolic, Modified Parabolic,
 * Fritsch-Butland, Akima, Kruger) use only $f$ values near $x_i$ to calculate
 * $f_i$. Non-local schemes (Spline with different boundary conditions) use all
 * ${f_i}$ values and obtain ${f_i}$ by solving a linear system of equations.
 * Local schemes produce $C^1$ interpolants, while the spline scheme generates
 * $C^2$ interpolants.
 * <p>
 * Hyman's monotonicity constraint filter is also implemented: it can be applied
 * to all schemes to ensure that in the regions of local monotoniticity of the
 * input (three successive increasing or decreasing values) the interpolating
 * cubic remains monotonic. If the interpolating cubic is already monotonic, the
 * Hyman filter leaves it unchanged preserving all its original features.
 * <p>
 * In the case of $C^2$ interpolants the Hyman filter ensures local monotonicity
 * at the expense of the second derivative of the interpolant which will no
 * longer be continuous in the points where the filter has been applied.
 * <p>
 * While some non-linear schemes (Modified Parabolic, Fritsch-Butland, Kruger)
 * are guaranteed to be locally monotone in their original approximation, all
 * other schemes must be filtered according to the Hyman criteria at the expense
 * of their linearity.
 * <p>
 * See R. L. Dougherty, A. Edelman, and J. M. Hyman, "Nonnegativity-,
 * Monotonicity-, or Convexity-Preserving CubicSpline and Quintic Hermite
 * Interpolation" Mathematics Of Computation, v. 52, n. 186, April 1989, pp.
 * 471-494.
 * <p>
...
public class CubicInterpolation extends AbstractInterpolation {
     ...
     * <p>
     * Interpolations are not instantiated directly by applications, but via a
     * factory class.
     *
     * @see Cubic
     */
    ...
    public enum DerivativeApprox {
        /**
         * Spline approximation (non-local, non-monotone, linear[?]). Different
         * boundary conditions can be used on the left and right boundaries: see
         * BoundaryCondition.
         */
        Spline,
        /**
    ...
    public enum BoundaryCondition {
        ...
        Periodic,
        /**
         * Match end-slope to the slope of the cubic that matches the first four
         * data at the respective end
         */
        Lagrange;
    }
