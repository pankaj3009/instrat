...
abstract public class AbstractSolver1D<F extends Ops.DoubleOp> {
    ...
    //
    /**
     * This method returns the zero of the function <code>f</code>, determined
     * with the given accuracy <code>epsilon</code>; depending on the particular
     * solver, this might mean that the returned <code>x</code> is such that
     * <code>|f(x)| < epsilon f</code>, or that <code>f |x-\xi| < \epsilon \f$ where \f$ \xi \f$
     * </code> is the real zero.
     * <p>
     * This method contains a bracketing routine to which an initial guess must
     * be supplied as well as a step used to scan the range of the possible
     * bracketing values.
     */
     ...
     *
     * @latex$ f }, determined with the given accuracy {
     * @latex$ \epsilon }; depending on the particular solver, this might mean
     * that the returned {
     * @latex$ x } is such that {
     * @latex$ |f(x)| < \epsilon }, or that {@latex$ |x-\xi| < \epsilon } where
     * {@latex$ \xi } is the real zero. <p>
     * An initial guess must be supplied, as well as two values {
     * @latex$ x_\mathrm{min} } and {
     * @latex$ x_\mathrm{max} } which must bracket the zero (i.e., either {
     * @latex$ f(x_\mathrm{min}) \leq 0 \leq f(x_\mathrm{max}) }, or {
    ...
    private double enforceBounds(final double x) {
        ...
        if (upperBoundEnforced && x > upperBound) {
            return upperBound;
        }
        return x;
    }
