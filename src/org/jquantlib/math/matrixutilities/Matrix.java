 ...
 * <p>
 * Performance of multidimensional arrays is a big concern in Java. This is
 * because multidimensional arrays are stored as arrays of arrays, spanning this
 * concept to as many depths as necessary. In C++, a multidimensional array is
 * stored internally as a unidimensional array where depths are stacked together
 * one after another. A very simple calculation is needed in order to map
 * multiple dimensional indexes to an unidimensional index.
 * <p>
 * This class provides a C/C++ like approach of internal unidimensional array
 * backing a conceptual bidimensional matrix. This mapping is provided by method
 * <code>op(int row, int col)</code> which is responsible for returning the
 * physical address of the desired tuple <row,col>, given a certain access
 * method.
 * <p>
 * The mentioned access method is provided by a concrete implementation of
 * {@link Address} which is passed to constructors. Doing so, it is possible to
 * access the same underlying unidimensional data storage in various different
 * ways, which allows us obtain another Matrix (see method
 * {@link Matrix#range(int, int, int, int) and alike}) from an existing Matrix
 * without any need of copying the underlying data. Certain operations benefit a
 * lot from such approach, like {@link Matrix#transpose()} which presents
 * constant execution time.
 * <p>
 * The price we have to pay for such flexibility and benefits is that an access
 * method is necessary, which means that a the bytecode may potentially need a
 * dereference to a class which contain the concrete implementation of method
 * <code>op(int row, int col)</code>. In order to keep this cost at minimum,
 * implementation of access method keep indexes at hand whenever possible, in
 * order to being able to calculate the actual unidimensional index as fast as
 * possible. This additional dereference impacts performance more or less in the
 * same ways dereference impacts performance when a bidimensional array
 * (<code>double[][]</code>) is employed. Contrary to the bidimensional
 * implementation, our implementation can potentially benefit from bytecode
 * inlining, which means that calculation of the unidimensional index can be
 * performed potentially faster than calculation of address location when a
...
public class Matrix extends Cells<Address.MatrixAddress> implements Cloneable {
    ...
    //
    /**
     * This is a convenience method intended to return the physical address of
     * an element.
     * <p>
    ...
    /**
     * Retrieves an element of <code>this</code> Matrix which identified by
     * <i>(row, col)</i>
     *
    ...
    /**
     * Stores a value into an element of <code>this</code> Matrix which is
     * identified by <i>(row, col)</i>
     *
     * @param row coordinate
     * @param col coordinate
     */
    ...
    //
    /**
     * Returns the result of an addition of <code>this</code> Matrix and
     * <code>another</code> Matrix
     *
     * @param another
     * @return this
     */
    ...
    /**
     * Returns the result of a subtraction of <code>this</code> Matrix and
     * <code>another</code> Matrix
     *
     * @param another
     * @return this
    ...
    /**
     * Returns the result of a multiplication of <code>this</code> Matrix by a
     * <code>scalar</code>
     *
     * @param scalar
     * @return this
    ...
    /**
     * Returns the result of a division of <code>this</code> Matrix by a
     * <code>scalar</code>
     *
     * @param scalar
     * @return this
    ...
    //
    /**
     * Returns the result of addition of <code>this</code> Matrix and
     * <code>another</code> Matrix
     *
     * @param another
    ...
    //
    /**
     * Returns an Array which represents the multiplication of <code>this</code>
     * Matrix by an Array
     *
    ...
    /**
     * Returns a Matrix which represents the multiplication of <code>this</code>
     * Matrix and <code>another</code> Matrix
     *
     * @param another
