 ...
 *
 * <p>
 * <b>More Details about constructors:</b>
 * <p>
 * There are three ways in which a term structure can keep track of its
 * reference date:
 * <li>such date is fixed;</li>
 * <li>such date is determined by advancing the current date of a given number
 * of business days;</li>
 * <li>such date is based on the reference date of some other structure.</li>
 *
 * <p>
 * Case 1: The constructor taking a date is to be used. The default
 * implementation of {@link TermStructure#referenceDate()} will then return such
 * date.
 *
 * <p>
 * Case 2: The constructor taking a number of days and a calendar is to be used
 * so that {@link TermStructure#referenceDate()} will return a date calculated
 * based on the current evaluation date and the term structure and observers
 * will be notified when the evaluation date changes.
 *
 * <p>
 * Case 3: The {@link TermStructure#referenceDate()} method must be overridden
 * in derived classes so that it fetches and return the appropriate date.
 *
...
public abstract class AbstractTermStructure implements TermStructure {
     ...
     * <p>
     * Case 1: The constructor taking a date is to be used. The default
     * implementation of {@link TermStructure#referenceDate()} will then return
     * such date.
     *
     * <p>
     * Case 2: The constructor taking a number of days and a calendar is to be
     * used so that {@link TermStructure#referenceDate()} will return a date
     * calculated based on the current evaluation date and the term structure
     * and observers will be notified when the evaluation date changes.
     *
     * <p>
     * Case 3: The {@link TermStructure#referenceDate()} method must be
     * overridden in derived classes so that it fetches and return the
     * appropriate date.
     */
    private JDate referenceDate;

    /**
     * Beware that this variable must always be accessed via
     * {@link #dayCounter()} method. Extended classes have the option to
     * redefine semantics of a day counter by keeping their own private
     * dayCounter variable and providing their own version of
     * {@link #dayCounter()} method. When extended classes fail to provide their
     * version of {@link #dayCounter()} method, <i><b>this</b>.getDayCounter</i>
     * must throw an {@link IllegalStateException} because the private variable
     * dayCounter was never initialised.
     *
     * @see #getDayCounter
     */
    ...
    //
    /**
     * Beware that this variable must always be accessed via {@link #calendar()}
     * method. Extended classes have the option to redefine semantics of a
     * calendar by keeping their own private calendar variable and providing
     * their own version of {@link #calendar()} method. When extended classes
     * fail to provide their version of {@link #calendar()} method,
     * <i><b>this</b>.getCalendar</i>
     * must throw an {@link IllegalStateException} because the private variable
     ...
     * {@link TermStructure#referenceDate()} in derived classes so that it
     * fetches and return the appropriate reference date. This is the <i>Case
     * 3</i> described on the top of this class.
     *
     ...
     *
     * <p>
     * This constructor takes a date to be used. The default implementation of
     * {@link TermStructure#referenceDate()} will then return such date. This is
     * the <i>Case 1</i> described on the top of this class.
     *
     * @see TermStructure documentation for more details about constructors.
     */
     ...
     *
     * <p>
     * This constructor takes a number of days and a calendar to be used so that
     * {@link TermStructure#referenceDate()} will return a date calculated based
     * on the current evaluation date and the term structure. This class will be
     * notified when the evaluation date changes. This is the <i>Case 2</i>
