 ...
 * <p>
 * This term structure is bootstrapped on a number of interest rate instruments
 * which are passed as a vector of handles to RateHelper instances. Their
 * maturities mark the boundaries of the interpolated segments.
 * <p>
 * Each segment is determined sequentially starting from the earliest period to
 * the latest and is chosen so that the instrument whose maturity marks the end
 * of such segment is correctly repriced on the curve.
 *
 * @note The bootstrapping algorithm will raise an exception if any two
 * instruments have the same maturity date.
 *
 *
...
public class PiecewiseYieldCurve<
                T extends Traits, I extends Interpolator, B extends Bootstrap>
