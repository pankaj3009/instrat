 ...
 * {
 *
 * @latex[ dr_t = (\theta(t) - \alpha r_t)dt + \sqrt{r_t}\sigma dW_t }
 *
...
public class ExtendedCoxIngersollRoss extends CoxIngersollRoss {
     ...
     * <p>
     * {
     * @latex[ \varphi(t) = f(t) - \frac{2k\theta(e^{th}-1)}{2h+(k+h)(e^{th}-1)}
     * - \frac{4 x_0 h^2 e^{th}}{(2h+(k+h)(e^{th}-1))^1} } where {
     * @latex$ f(t) } is the instantaneous forward rate at {
     * @latex$ t } and {
     * @latex$ h = \sqrt{k^2 + 2\sigma^2} }.
     *
     * @author Praneet Tiwari
     */
     ...
     * {
     *
     * @latex[ r_t = \varphi(t) + y_t^2 } where \f$ \varphi(t) \f$ is the
     * deterministic time-dependent parameter used for term-structure fitting
     * and {
     * @latex$ y_t } is the state variable, the square-root of a standard CIR
     * process.
     *
     * @author Praneet Tiwari
     */
